...
const Navbar = () => {
    ...
    const [isSticky, setIsSticky] = useState<boolean>(route.length > 2 ? true : false)
            ...
            <nav dir="ltr" style={{ animation: route.length == 1 && isSticky ? ' slideDown 0.35s ease-out' : 'none', boxShadow: isSticky ? '0 5px 16px rgba(0, 0, 0, 0.1)' : 'none' }} className={`${isSticky ? '!top-0 !fixed z-[999] left-0 w-full text-black bg-white !max-w-full' : '  z-30 absolute  right-0 left-0 Container'} `}>
                <div className={`${!isSticky ? 'py-4' : 'py-2'} Container flex justify-between xl:!px-0 px-3 lg:!px-6 gap-2 sm:!gap-2 lg:!gap-5`}>
                    ...
                    */}
                        ...
                        <div className={` ${isSticky && 'border border-solid'} border-[#0000005c] flex gap-2 bg-[#ffffff54] items-center pr-3 pl-2 p-1 rounded-full text-3xl`}>
                            <FaRegCircleUser className='text-gray-500' />
                            <RxHamburgerMenu className=' text-xl cursor-pointer' onClick={() => setShowMenu(true)} />
                        </div>
                    </section>
                </div>
            </nav>
